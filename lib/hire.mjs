...
export function hire(root, { name, kind, description = '', agreed = null, now = () => Date.now() }) {
  // ⛔ 40 IS NOT ARBITRARY. A desk name goes into a directory path, a port map, a docs table
  // and a test name. A 200-character one was accepted until somebody tried it: paths near
  // the filesystem limit, and a docs table nobody can read.
  if (!/^[a-z][a-z0-9-]{0,39}$/.test(name)) {
    throw new Error(
      `"${String(name).slice(0, 44)}" is not a desk name. Lowercase letters, digits and hyphens, ` +
      `starting with a letter, 40 characters at most.`)
  }
